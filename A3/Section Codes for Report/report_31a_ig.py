...
class DecisionTree:
        ...
        for w in range(indices):
            ...
            for thres in thresholds:
                ...
                if(avg_entropy > max_avg_entropy or it == 1):
                    ...
                    temp = thres
                    it = 2
            vector.append(list([max_avg_entropy,temp]))
        return vector
    ...
    def ID3(self,depth,feature_vector, output_vector, parent_entropy):
        ...
        gain_max = -1
        k = 0
        self.info = self.entropies(len(feature_vector[0]),feature_vector,output_vector)
        ...
        if(total_left < self.min_samples_split or depth == self.max_depth-1):
            ...
            left_child = self.ID3(depth+1,feature_cap,output_cap,t1+t2)
        ...
        if(total_right < self.min_samples_split or depth == self.max_depth-1):
            ...
            right_child = self.ID3(depth+1,feature_cap,output_cap,t1+t2)
...
begin = time.time()
DT = DecisionTree(10,7,feature_vector,output_vector)#20,7
