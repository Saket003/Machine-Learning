...
class DecisionTree:
    ...
    def ID3(self,depth,unused_indices,feature_vector, output_vector, parent_entropy):
        ...
        unused_indices_copy = unused_indices.copy()
        unused_indices_copy.remove(k)
        ...
        if(total_left < self.min_samples_split or depth == self.max_depth-1):
            ...
            t1 = (ones0/total0)*np.log2(total0/ones0) if (ones0 != 0) else 0
            t2 = (zeros0/total0)*np.log2(total0/zeros0) if (zeros0 != 0) else 0
            left_child = self.ID3(depth+1,unused_indices_copy,feature_vector,output_vector,t1+t2)
        ...
        if(total_right < self.min_samples_split or depth == self.max_depth-1):
            ...
            t1 = (ones0/total0)*np.log2(total0/ones0) if (ones0 != 0) else 0
            t2 = (zeros0/total0)*np.log2(total0/zeros0) if (zeros0 != 0) else 0
            right_child = self.ID3(depth+1,unused_indices_copy,feature_vector,output_vector,t1+t2)
